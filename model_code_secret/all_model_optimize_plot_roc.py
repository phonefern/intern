...
def train_model(use_smote=True):
    # Read data
    # df = pd.read_csv(r"D:\project_kiosk\model_code_secret\new_test_set.csv")
    ...
    with open(output_file, "w") as f:
        ...
        for name in models.keys():
            ...
            f.write(f"  Average AUC: {roc_auc_score_value[name]:.2f}\n")
    ...
    # save_path = "D:\project_kiosk\model_code_secret\model_joblib"  
    # os.makedirs(save_path, exist_ok=True)
    # joblib_file = os.path.join(save_path, f"{name}.joblib")
    # joblib.dump(model, joblib_file)
    # print(f"Model {name} saved to {joblib_file}")
